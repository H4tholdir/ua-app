...
**⚠️ Criterio QA fallito (non corretto, Task 4 = solo verifica):** bottone "Rimuovi" 18px, sotto i 44px richiesti esplicitamente dal piano — decidere se fixare prima del merge.

**Prossimo step:** valutare merge di B8 (3/5), poi iniziare **B8 (4/5) — /rete/nuova** (ciclo brainstorming → spec → piano → implementazione, worktree dedicato). `POST /api/rete` esiste già e funziona (auto-aggiunge il lab creatore come admin) — manca solo la UI.
