...
🛑 **Branch `ondata-a-denti-colore`**, repo principale, **37 commit avanti a `main`. Niente in
...
sempre-201.** Suite intera **3477 verdi · tsc 0 · eslint 0 · `next build` ok · DB alla baseline
(294 lavori, 0 denti).** 🔑 `denti` presente ma non-array → **422**, non un «nessun dente»
...
🔴 **RITROVAMENTI T9 fuori mandato — riferiti, non toccati** (dettaglio in §5-bis dell'handoff):
...
(`ruolo:'pippo'` → 422 sul PUT, **500** sul POST): estrarre un modulo unico, **assegnato a T11/T12** ·
③ `crea-lavoro.ts:164,191-196` **usa ancora la PATCH fail-soft**: il percorso atomico esiste e
nessuno lo chiama — conferma dal vivo che **T10 senza T11 perde i denti in silenzio** · ④ il wizard
scrive `colore_dente` (colonna legacy), il percorso nuovo la coppia `colore_scala`/`colore_codice`:
due fonti dello stesso fatto (R3) · ⑤ `anno = new Date().getFullYear()` accanto a `oggiRomaISO()`:
alla mezzanotte del 1° gennaio i due non concordano (finestra stretta, non chiuso).
