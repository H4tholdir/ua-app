...
**⚠️ Finding QA — criterio esplicito del piano (Step 4) NON soddisfatto:** il piano richiedeva touch target ≥44px per "Rimuovi", "+ Aggiungi rischio" e "Salva modifiche". Verificato: "+ Aggiungi rischio" 44px e "Salva modifiche" 48.5px sono conformi; **il bottone testuale "Rimuovi" (link rosso senza padding esplicito in `RischiEditor.tsx`) misura solo 18px di altezza**, sotto il minimo richiesto, su tutti e 3 i viewport. Introdotto da questo branch stesso (non ereditato). Non corretto in questa sessione (Task 4 = "nessuna modifica di codice"): fix banale (aggiungere `minHeight: 44` + padding al bottone), lasciato aperto per decisione esplicita di Francesco (mergiare con questo gap noto, o fixarlo prima del merge).
