# UÀ — Project Memory
**Ultimo aggiornamento:** 4 luglio 2026 — **S4 (Email template branding) — HTML finalizzato, in attesa di applicazione manuale** (commit `01e915c`, pushato su `origin/main`). Rebrand DS v2.3 dei 3 template Supabase Auth (Confirm Signup, Reset Password, Invite User): palette Warm Panna hardcoded in hex (i client email non leggono CSS custom properties), font `'DM Sans', Helvetica, Arial, sans-serif`, layout a tabelle inline (compatibilità Outlook). Due round di feedback di Francesco applicati: (1) logo reale in alto — sostituito il wordmark testuale con `ua-logo-email.png` (asset già esistente in `public/`, 120×120, servito in produzione da `https://uachelab.com/ua-logo-email.png`); (2) link di fallback sotto il bottone reso un vero `<a href="{{ .ConfirmationURL }}">` con testo integrale (`word-break: break-all`, non più troncato con "..."), poi nascosto sotto uno spoiler nativo `<details><summary>Il bottone non funziona?</summary>` (nessun JS disponibile in email; degrado sicuro — Apple Mail/client moderni lo rendono collassabile, Outlook desktop e alcune versioni Gmail lo mostrano già espanso, mai nascosto in modo irraggiungibile). Mockup (`docs/design/mockups/2026-07-04-s4-email-templates-branding.html`) verificato con screenshot Playwright in 3 round e approvato da Francesco. Decisione registrata in `docs/design/decisions/2026-07-04-s4-email-templates-branding.md`. HTML finale pronto in `docs/email-templates-supabase.md`. **Applicazione su Supabase Dashboard (https://supabase.com/dashboard/project/iagibumwjstnveqpjbwq/auth/templates) resta manuale**: nessuna sessione Chrome autenticata disponibile in questa sessione (`list_connected_browsers` vuoto) e nessun tool MCP Supabase espone la configurazione email di Auth (solo tabelle/migration/edge function/branch, non `mailer_templates_*`) — Francesco deve incollare i 3 blocchi HTML lui stesso, oppure connettere una sessione Chrome autenticata in una sessione futura perché Claude lo faccia direttamente.
