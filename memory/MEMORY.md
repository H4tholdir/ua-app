# UÀ — Project Memory
**Ultimo aggiornamento:** 3 luglio 2026 — **Pulizia git (commit `05cf155`):** `package.json`/`package-lock.json` avevano una modifica non committata da sessioni precedenti (dipendenza `server-only`, già importata in decine di file server ma mai aggiunta al manifest) — committata separatamente, working tree ora pulito. B8 (5/5, `/rete/[id]`) in corso: spec di design scritta e approvata (`docs/superpowers/specs/2026-07-03-b8-rete-id-design.md`, commit `1e6ee39`), piano di implementazione in scrittura. B8 (1/5, /magazzino/nuovo) chiuso e deployato su main (merge `a810c36`) — CTA magazzino ora funzionante. B8 (2/5, /listino/nuovo) chiuso e deployato su main (merge fast-forward, commit `2251b60`, pushato su `origin/main`) — CTA listino ora funzionante e gated per ruolo. B8 (3/5, /qualita/rischi/[id]) verificato, review finale whole-branch approvata (1 fix Minor applicato prima del merge) su branch dedicato (worktree `worktree-b8-rischi-id`, non ancora mergiato su `main` al momento di questo aggiornamento) — link "Modifica →" ora apre un editor a pagina intera funzionante con ricalcolo RPN server-side. B8 (4/5, /rete/nuova) chiuso e deployato su main (merge fast-forward, commit `e84257f`, pushato su `origin/main`) — bottone "Crea rete" ora apre un bottom sheet funzionante con guard server-side 1-rete-per-lab; QA ha trovato un bug reale trasversale (submit non cliccabile via touch/click a 390/768px per collisione z-index con la bottom-nav) risolto sullo stesso branch prima del merge. **Hotfix successivo (03/07/2026, branch `fix-listino-zindex`):** lo stesso bug di z-index, riprodotto identico in `ListinoNuovoSheet.tsx` (B8 2/5, già in produzione), corretto separatamente — vedi voce dedicata sotto.

---

## 0. STATO DEL PROGETTO — RE-AUDIT 02/07/2026 COMPLETATO, B1 RISOLTO, B2 RISOLTO, B7 RISOLTO, B8 (1/5) RISOLTO (03/07/2026), B8 (2/5) RISOLTO (03/07/2026), B8 (3/5) RISOLTO (03/07/2026), B8 (4/5) RISOLTO (03/07/2026)
