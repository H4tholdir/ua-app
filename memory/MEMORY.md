...
**Backlog non bloccante aperto (non B8, tracciato per follow-up):** test di route dedicati per `verify-admin-rete.ts` (attualmente unica barriera di isolamento tenant, priva di test — stesso pattern già accettato per `verify-titolare.ts`); check di esistenza rete nella force-add admin (404 invece di 500 su id inesistente); escaping HTML condiviso per `send-invito-rete-email.ts`/`send-invito-email.ts` (nessuno dei due esegue escaping di `reteNome`/`inviteUrl`/`labNome`, rischio basso — mittente è sempre un admin già autenticato); audit contrasto trasversale dei badge rainbow (`--c-purple` e altri, ~3.2:1 sotto soglia AA per testo piccolo); nessun componente dei Task 13-17 importa da `src/design-system/tokens.ts` (pattern di stili inline con `var(--token, #fallback)` consolidato in questo piano, non da correggere task-per-task ma con un unico refactor trasversale).
