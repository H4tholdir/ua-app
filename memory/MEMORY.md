...
## 0. STATO DEL PROGETTO — RE-AUDIT 02/07/2026 COMPLETATO, B1 RISOLTO, B2 IN PROGETTAZIONE (spec approvata)
...
**🟡 B2 — IN PROGETTAZIONE (02/07/2026), spec approvata, piano non ancora scritto.** Causa reale (non "due fonti mai riconciliate" come nel backlog originale): `lavori_partitario` non ha mai avuto un writer applicativo (0 righe anche in produzione, verificato su DB live), quindi 3 punti (Dashboard KPI, widget "clienti morosi" incl. `admin/labs/[id]/live`, widget Front Desk) mostrano sempre il prezzo pieno come insoluto. Discussione con Francesco ha allargato lo scope a una feature di business reale — "Contabilità Clienti" (sotto-progetto 1 di 3, gli altri due sono Preventivo e Riepilogo multicanale, fuori scope per ora): ledger pagamenti polimorfico (fattura o lavoro diretto), flag `lavori.decisione_fatturazione`, credito cliente con eccedenze/rimborsi, Scadenzario ampliato ai lavori diretti. Spec completa (validata anche da review indipendente, 3 gap architetturali corretti prima del piano): `docs/superpowers/specs/2026-07-02-contabilita-clienti-design.md` (commit `05f4f1b`). **B2 resta aperto come blocker fino al deploy di questo sotto-progetto — nessun fix parziale possibile** (i bucket di credito sono interdipendenti, vedi spec §5). **Prossimo step: invocare `superpowers:writing-plans` sullo spec approvato per produrre il piano di implementazione** (non ancora fatto, richiesto esplicitamente in una nuova sessione a contesto pulito).
