...
| 9 | Catena gesto in `Cassetta.tsx` (soglia 8px, timer 300ms): su Chrome Android il tap non arriva a `onTap`. `pointercancel` NON è gestito; possibile jitter > soglia + click sintetico non servito. L'onTap è cablato giusto (`PareteClient.tsx:281`: libera → sheet) — iOS è il comportamento corretto | lettura `Cassetta.tsx:107-200` |
