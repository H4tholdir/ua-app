...
- **Parete**: griglia a colonne fisse 3 (<768) / 4 (768-1279) / 6 (≥1280), ordine lineare per `posizione, created_at, id`. Ogni cella = `Cassetta` (§5.35): occupata → tray con miniatura (§5.36), targa e riga «n.{numero} · {dentista}», **tap → scheda lavoro**; libera → **tap → sheet cassetta**; in coda la cella tratteggiata «+ Nuova cassetta» → sheet.
- **Ricerca «che accende» (§8.3.7):** filtro client; le match si accendono (anello + elevazione), le non-match si spengono (opacity .3 + desaturazione) ma **restano tappabili** (mai `pointer-events:none`); mai solo colore; `aria-live` con l'esito; zero match → tutte spente + riga quieta «Niente per "…"». (Il pagliaio della ricerca è definito nella spec di design §5.1 — ratifica 22/07: ricerca «globale».)
...
7. **L'accensione (emendamento 21/07/2026, ondata A — ricerca Parete §5.35):** quando la ricerca «accende» una cassetta, anello blu 3px + lieve elevazione entrano `snappy`; le non-match si **spengono** in dissolvenza (opacity → .3 + desaturazione, `transition: opacity` sola). Reduced-motion: anello senza elevazione animata, dissolvenze 150ms (§8.4). L'acceso/spento non è mai solo colore (anello + opacità + `aria-current`, §12).
