...
- **Parete**: griglia a colonne fisse 3 (<768) / 4 (768-1279) / 6 (≥1280), ordine lineare per `posizione, created_at, id`. Ogni cella = `Cassetta` (§5.35): occupata → tray con miniatura (§5.36), targa 🔄 **SENZA il numero del lavoro** — spec redesign §2.3 RATIFICATA 23/07 («*il **numero lavoro esce dalla targa** → resta nello sheet e nella scheda*») e verbale `docs/design/decisions/2026-07-24-rete-gancetto-targa.md:23` («***MAI** numero lavoro/tipo in targa*»): la targa porta **dentista + paziente**. ⚠️ **Domanda APERTA, non sanata qui:** `src/components/ds/Cassetta.tsx:724-728` rimette il numero nel **nome accessibile** (non nella targa dipinta), con una lettura dell'implementatore — «*il ratificato che lo vieta parla della TARGA*» — che **nessuna decisione conferma**. È l'unica divergenza dell'audit del 03/08 **senza** una ratifica dietro: va decisa, non emendata.
