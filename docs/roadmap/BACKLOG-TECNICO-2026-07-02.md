...
### N5. `generaFatturaPA` hardcoda TD01 — blocca le note di credito TD04 (IMPORTANT, emerso in review Ondata 2, 11/07/2026)
Il tracciato XML FatturaPA e il PDF di cortesia dichiarano sempre `TipoDocumento TD01` (fattura), qualunque sia il documento. Oggi il percorso è inerte (nessun flusso emette TD04), ma il giorno in cui si emetteranno **note di credito** l'XML dichiarerebbe il tipo sbagliato = documento fiscalmente invalido. **Vincolo bloccante:** da fixare PRIMA di costruire qualsiasi percorso di emissione TD04 (parametrizzare `tipo_documento` in `generaFatturaPA` + template cortesia + test content-check). **Destinazione:** task dedicato, prerequisito della feature «note di credito».
