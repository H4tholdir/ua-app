...
6. **✅ Copertura comportamentale RPC** — nel verificarla è stato scoperto un **bug critico** (vedi sotto), risolto come hotfix separato. Nessuna infrastruttura di test di integrazione locale costruita in questa sessione (il progetto non ha `supabase/config.toml`/docker) — gap strutturale segnalato come backlog a parte, la verifica per questo fix è stata manuale via transazioni `BEGIN/ROLLBACK` sul DB live.
