...
### N8. `urgencyPillBg`/`urgencyPillBorder` concatenano alpha su una stringa `var()` = CSS invalido (scoperto durante N4, 14/07/2026)
In `src/components/features/scadenzario/estratto-conto-shared.ts` gli helper `urgencyPillBg`/`urgencyPillBorder` costruiscono il colore di sfondo/bordo delle urgency pill concatenando un valore alpha direttamente su una stringa `var(--token)` (es. `` `${var(--c-red)}${alpha}` ``) — sintassi CSS non valida, il browser scarta la dichiarazione e il background risulta trasparente. **Pre-esistente**, non introdotto da N4 (toccato solo di striscio nello stesso file per il badge divergenza, senza modificare questi due helper). **Destinazione:** task dedicato — usare `color-mix()` o token RGBA dedicati invece di concatenazione stringa su `var()`; stessa famiglia di fix già applicata al nuovo `--c-amber-ink` in N4.
