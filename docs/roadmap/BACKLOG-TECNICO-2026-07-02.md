...
| B18 | Hardening trasversale post-B3 (8 finding non bloccanti) | ⏳ | | **Priorità esplicita di Francesco: da fare PRIMA di ogni altro item non ancora iniziato in questa tabella.** Scoperto 04/07/2026 durante l'esecuzione di B3 — vedi dettaglio sotto |
...
### B3. ✅ RISOLTO (04/07/2026, branch `worktree-b3-cicli-produzione`, 18 commit `3d5f5a0..ad050cb`, non ancora mergiato su `main`) — Cicli di produzione non generavano mai fasi per i lavori nuovi
...
### B18. Hardening trasversale post-B3 — 8 finding non bloccanti da chiudere prima di ogni altro item
**Fonte:** emersi durante l'esecuzione di B3 (review task-scoped individuali + review finale whole-branch + QA manuale), nessuno bloccante per il merge di B3 stesso (già mergiato). **Priorità esplicita di Francesco:** questo item va risolto per primo, prima di qualunque altro item ⏳ in questa tabella.

1. **`.or()` PostgREST con `q` interpolato senza escape** in `GET /api/cicli` (`src/app/api/cicli/route.ts`) e `GET /api/fasi-produzione/ricerca` (`src/app/api/fasi-produzione/ricerca/route.ts`), più lo stesso pattern identico pre-esistente in `src/app/api/clienti/route.ts:41` e `src/app/api/listino/route.ts:46`. Nessun rischio di leak cross-tenant (verificato: lo scoping lab è un filtro `.eq()` separato, sempre combinato in AND da PostgREST indipendentemente dal contenuto di `q`), ma una ricerca con virgola/parentesi produce un errore di parsing invece di risultati validi. Fix: escapare i caratteri riservati PostgREST (`, . : * ( )`) prima dell'interpolazione, su tutte e 4 le route insieme per coerenza.
2. **Indice `UNIQUE (ciclo_id, codice_fase)` su `fasi_produzione` non-partial** (non `WHERE deleted_at IS NULL`) — pre-esistente. Riusare un `codice_fase` appena rimosso (stessa richiesta batch o richiesta successiva) collide con la riga soft-deletata → la RPC `salva_fasi_ciclo_atomico()` fallisce in modo pulito con 500 (grazie all'atomicità, migliore del comportamento precedente che avrebbe fatto scritture parziali). Fix: rendere l'indice parziale (`WHERE deleted_at IS NULL`) via migration, oppure riordinare la RPC per eseguire il soft-delete prima degli insert.
3. **Dedup mancante per `codice_fase`** nei risultati di `GET /api/fasi-produzione/ricerca` quando lo stesso codice è usato in più cicli del lab — `CicloFasiEditor.tsx` usa `key={r.codice_fase}` per il rendering dei risultati, causando key collision React (solo warning console, nessun malfunzionamento). Fix: `distinct on codice_fase` nella query, o chiave basata sull'indice nel render.
4. **`var()` senza fallback esadecimale** in `CicloComboBox.tsx` (`boxShadow: 'var(--sh-b)'`, `transition: 'background var(--tr)'`) — innocuo perché i token sono definiti globalmente, ma incoerente col pattern `var(--token, #fallback)` usato ovunque altrove nello stesso file.
5. **Test con mock che non verificano gli argomenti esatti di `.eq('laboratorio_id', ...)`** in diverse route create durante B3 (es. `cicli-route.test.ts`, `cicli-fasi-patch-route.test.ts`) — lo scoping tenant è corretto nel codice, ma un refactor che lo rimuovesse per errore non farebbe fallire alcun test esistente. Fix: rafforzare i mock per asserire gli argomenti esatti passati a `.eq()`.
6. **Copertura di test solo sulla mappatura, non sul comportamento della RPC** — i test di `PATCH /api/cicli/[id]/fasi` verificano status code e parametri passati a `svc.rpc(...)`, non il comportamento SQL reale di `salva_fasi_ciclo_atomico()` (verificato solo per lettura del codice + applicazione in produzione). Fix: valutare un test di integrazione contro un DB Supabase locale/di staging che eserciti realmente la funzione.
7. **Race condition nota e accettata** in `LavoroFormClient.handleUpdateFase` (`src/components/features/lavori/LavoroFormClient.tsx`) — la variabile `previous` per il rollback è catturata per-call dalla closure su `fasi`; due `PATCH` quasi simultanee sulla stessa fase (es. doppio tap rapido) potrebbero far sì che il rollback della prima sovrascriva l'aggiornamento della seconda già in volo. Bassa probabilità (richiede un doppio tap dell'utente sullo stesso elemento), non riprodotta in QA. Fix: usare un contatore di richieste in-flight per fase (stesso pattern già usato in `CicloComboBox`/`ClienteComboBox` per la ricerca) invece di un semplice snapshot pre-fetch.
8. **2 hydration mismatch pre-esistenti in componenti condivisi**, osservati durante la QA di B3 ma mai toccati da nessun task B3:
   - `BottomNavPill` — già noto e documentato in precedenza in `memory/MEMORY.md` (tooltip del bottone "+" renderizzato diversamente server/client).
   - `ThemeToggleButton` — osservato per la prima volta durante questa QA: `aria-label` differisce tra server (sempre "Passa al tema scuro") e client (riflette il tema realmente attivo dopo l'idratazione, letto da preferenza salvata/`prefers-color-scheme`).
   Fix: entrambi richiedono che il valore iniziale lato server rifletta lo stesso stato che il client userà al primo render (es. leggere il tema da un cookie leggibile server-side, invece che solo da `localStorage`/media query lato client).

**Effort:** non stimato — punti 1/3/4 sono piccoli fix mirati (poche ore in totale); punto 2 richiede una migration; punti 5/6 sono lavoro di test; punto 7 richiede una piccola modifica di stato; punto 8 richiede capire come il tema viene attualmente persistito e propagato server-side.
