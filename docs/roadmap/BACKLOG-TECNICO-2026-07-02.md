...
| B7 | "Invita tecnico" irraggiungibile da UI | ✅ | 03/07/2026 · `fe81be6` | Riga tabella disallineata dalla sezione narrativa da tempo (già risolto e mergiato su `main`, verificato 09/07/2026) — vedi dettaglio sotto |
| B8 | 5 route CRUD → 404 | ✅ | 04/07/2026 · `57aa52c`+`c8c3182` | 5/5 route risolte e mergiate su `main` (verificato 09/07/2026, backlog mai aggiornato): magazzino/nuovo, listino/nuovo, qualita/rischi/[id], rete/nuova, rete/[id] (quest'ultima implementata 04/07 senza passare da un aggiornamento BP-1 tracciato) — vedi dettaglio sotto |
...
| B14 | `tecnici.compenso_base` ambiguo | ✅ | 09/07/2026 | `tipo_compenso` esposto in UI (form tecnico) e usato per disambiguare la card Compenso (fisso/percentuale/per_lavorazione/non specificato) — vedi dettaglio sotto e `memory/MEMORY.md` §0 |
...
| B22 | Migration history remota: 26 migration fondative pre-02/07/2026 (`001_...` → `20260525...`) mai registrate in `supabase_migrations.schema_migrations` | ✅ | 09/07/2026 · `ee52f09` | Risolto come P1 della sequenza binario unico: repair una-alla-volta con verifica contenuti contro DB live (53 check oggetti DDL). 13 file rinominati a versioni univoche (collisioni 20260517/18/20/21). Scoperto e sanato 1 file MAI applicato davvero (`realtime_replica_identity`: `REPLICA IDENTITY FULL` mancante su `lavori`/`fatture` — applicato al DB live). `migration list` pulito, `db push --dry-run` "up to date". Vedi dettaglio sotto |
...
### B8. ✅ RISOLTO (5/5, ultima route `rete/[id]` confermata su `main` il 04/07/2026 · `57aa52c`+`c8c3182`, verificato retroattivamente il 09/07/2026) — 5 route CRUD portano a pagine 404
**Fonte:** [Sis]
...
### B14. ✅ RISOLTO (09/07/2026, worktree `b14-tipo-compenso-tecnico`, 3 commit, mergiato e deployato) — `tecnici.compenso_base` ancora semanticamente ambiguo
**Fix applicato:** `tipo_compenso` (già in schema/DB live/tipi generati, mai collegato a nulla) esposto in `TecnicoEditInline.tsx` (select + input `compenso_base` con `min="0"`, backend già pronto — allowlist PATCH/POST verificata, nessuna modifica route necessaria). `ProduttivitaTecnico.tsx` reso condizionale a 3 stati: `fisso` → stipendio statico senza barra; `percentuale`/`per_lavorazione` → barra target invariata (`compenso_maturato` è calcolato uguale per tutti i tipi — somma `listino.compenso_tecnico × quantità` — quindi la barra resta valida solo per questi 2 casi); non classificato → nudge "Tipo compenso non specificato" (decisione esplicita di Francesco). Un secondo reviewer ha scoperto che l'ordine iniziale dei rami faceva fallire la nudge proprio per il caso più comune (tecnico mai configurato, `tipo_compenso` E `compenso_base` entrambi null — lo stato di ogni tecnico esistente) — corretto e verificato dal vivo. Nessuna migration necessaria. QA browser su 4 stati + salvataggio reale end-to-end sul lab E2E. `tsc`/`vitest` (730/4 skipped, era 712)/`next build`/DS-compliance puliti. Review indipendente: "Ready to merge: Yes". Dettaglio completo: `memory/MEMORY.md` §0.
...
**Ricognizione aggiuntiva (09/07/2026, [SWE], propedeutica a una futura sessione — nessuna riga di codice toccata):** esiste già in schema (`supabase/schema.sql:334`, mai in una migration tracciata — stesso gap di B22) una colonna gemella **mai collegata a nulla**: `tecnici.tipo_compenso TEXT CHECK (IN ('fisso','percentuale','per_lavorazione'))`. Presente nel tipo dominio (`src/types/domain.ts:704`) ma **zero occorrenze in tutto `src/components/` e `src/app/`** — nessun form la imposta, nessuna pagina la legge. `compenso_base` (usato solo come "target mensile" in `ProduttivitaTecnico.tsx`) viene quindi sempre interpretato allo stesso modo indipendentemente da quale `tipo_compenso` avrebbe il tecnico. Questo è quasi certamente il meccanismo di disambiguazione già previsto in fase di design originale ma mai completato — **non serve una nuova migration per la semantica di base**, il campo classificatore esiste già. Scope concreto per la prossima sessione:
1. Conferma con Filippo: i 3 valori (`fisso`/`percentuale`/`per_lavorazione`) coprono i casi reali? `compenso_base` significa cose diverse per ciascuno (stipendio fisso mensile / target su cui calcolare percentuale / tariffa a lavorazione)?
2. Se confermato: aggiungere `tipo_compenso` al form tecnico (creazione/modifica, oggi assente), wired su `POST /api/tecnici`/`PATCH /api/tecnici/[id]` (già nell'allowlist? verificare — `src/app/api/tecnici/[id]/route.ts:14` lista solo `compenso_base`, andrebbe aggiunto `tipo_compenso`).
3. `ProduttivitaTecnico.tsx:307` — label/interpretazione di `compensoBase` condizionale a `tipo_compenso` (oggi genericamente "target mensile" per tutti e 3 i casi).
4. Migration eventuale solo se serve un default espicito o un vincolo `NOT NULL` — da valutare dopo la decisione con Filippo, non prima.
...
**✅ RISOLTO (09/07/2026, commit `ee52f09`, eseguito come P1 della sequenza binario unico):**
- **26 file pendenti** (non ~25): `001_commercial_infra` → `20260525000002_invite_atomic`. Verifica preliminare dei contenuti contro il DB live (`iagibumwjstnveqpjbwq`) con 53 check batch su oggetti DDL (tabelle, funzioni, colonne, indici, constraint, extension): 51 OK, `lavori_partitario` assente come atteso (drop registrato 02/07), un solo vero gap (sotto).
- **Complicazione non anticipata dal backlog — versioni in collisione:** la CLI deriva la versione dal prefisso del filename e la history table ammette una sola riga per versione, ma 13 file condividevano 4 versioni (3× `20260517`, 3× `20260518`, 4× `20260520`, 3× `20260521`). Risolto rinominando i 13 file a versioni univoche (`20260517000001`…`20260521000003`), ordine relativo preservato (stesso ordine alfabetico che `db reset` avrebbe già applicato), SQL invariato byte-per-byte, solo `git mv`. Aggiornato l'unico riferimento stale (commento in `src/app/api/notifications/subscribe/route.ts`).
- **Scoperta reale: `20260521000002_realtime_replica_identity.sql` non era MAI stato applicato al DB live** — `relreplident` era `'d'` (default) su `lavori` e `fatture` invece di `'f'`. Gap attivo in produzione: entrambe le tabelle sono nella publication `supabase_realtime` e `useRealtimeNotifiche.ts` confronta `payload.old` (senza FULL, `old` contiene solo la PK → falsi positivi nelle notifiche). Applicati i 2 `ALTER TABLE ... REPLICA IDENTITY FULL` al DB live (contenuto esatto del file, reversibili), verificato `relreplident='f'`, poi registrato in history.
- **Repair eseguito una versione alla volta** via `supabase migration repair --status applied <version>` (26 comandi, mai `--include-all`), righe verificate in `schema_migrations` con nome corretto dopo ogni batch.
- **Done verificato:** `supabase migration list` → 50/50 voci `local`=`remote`, zero pendenti; `supabase db push --dry-run` → "Remote database is up to date". Nota: `MANUAL_000_auth_helpers.sql` resta fuori history per design (pattern filename non valido, la CLI lo salta sempre).
...
**Piano:** R1 = region `dub1` (config-only, reversibile) → re-misura → R2 = riduzione round-trip (getClaims middleware → helper `getLabContext` con cache() → Promise.all → consolidamento route; percorso GRANDE, tocca auth) → R3 = Speed Insights/Server-Timing/pg_stat_statements + Supabase Pro al primo cliente pagante. Nuovi item scoperti dal panel: **N11** (sicurezza `deleted_at`) e **N12** (atomicità route prove) — vedi §N.
**✅ R1 ESEGUITO E VERIFICATO (17/07/2026, gate esplicito di Francesco, commit `441948b`, deploy confermato `fra1::dub1`):** smoke PASS (login, navigazioni, write-path PATCH no-op 200/283ms, webhook Stripe 400-atteso/195ms); re-misura stesso script: **mediana TTFB 812→354ms (−56%), login→dashboard 5.189→2.486ms (−52%), API GET −70/−80%** (`/api/clienti` 845→188ms). Outlier: `/fatture/riconciliazioni` −8% (dominata dalle proprie query → prioritaria in R2d). Residui per il budget (TTFB ≤300ms, login ≤2s): **R2**. Da osservare: primo invio PEC reale post-R1 (IP di uscita cambiato). Tabella completa before/after in `docs/roadmap/P0-PERF-DIAGNOSI-2026-07-17.md` §6.
...
### N8. ✅ RISOLTO (14/07/2026, `main` `377ad27`, deployato) — alpha concatenato su `var()` = CSS invalido
In `src/components/features/scadenzario/estratto-conto-shared.ts` gli helper `urgencyPillBg`/`urgencyPillBorder` costruivano sfondo/bordo delle urgency pill concatenando un valore alpha su una stringa `var(--token)` (`` `${var(--…)}22` ``) — CSS invalido, dichiarazione scartata, sfondo trasparente. **Pre-esistente**, non introdotto da N4.
**Fix (branch `fix-urgency-pill-color-mix`, merge `377ad27`):** convertiti a `color-mix(in srgb, var(--…) 13%/27%, transparent)`. Corretti anche gli altri 3 siti con lo stesso difetto trovati durante la scansione: `ScadenzarioList.tsx` (pill inline), `src/app/portale/[token]/page.tsx:77` (badge stato `in_prova`), `src/app/(app)/qualita/page.tsx:286` (boxShadow incidenti). **Verificato empiricamente** (getComputedStyle): tint VISIBLE in light+dark per tutti gli stati/gravità; il vecchio pattern riproduceva `rgba(0,0,0,0)`. CI+CD verdi, smoke uachelab.com 200.
...
- **«N10 polish»** (review finale whole-branch, tutti ≤10 righe, nessuno tocca la semantica d'invio): dedup allowlist ruoli UI/server (`RUOLI_INVIO_PEC` in modulo condiviso directive-free) · audit log operatore anche sul ramo `invia_pec` di `/xml` · `.eq('laboratorio_id')` sul re-fetch post-invio · fallback contratto 200 se re-fetch errora · scroll-lock body con bottom sheet aperto · distinguere "PEC unknown" da "non configurata" su errore infra della query lab · test dedicato ramo claim-error di `/xml` · sub-line cliente/importo + relabel «Riprova invio» nel dialogo (fedeltà mockup, richiede nuove prop). 🟢
...
### D-11. Purga per-tenant: fix DI CLASSE + 3 tabelle già orfane in `admin_delete_laboratorio` (aperto 21/07/2026, dall'ondata «Parete delle Cassette») 🟠 sicurezza/GDPR — DECISIONE DA PRENDERE (panel proprio)
**Origine:** R-3 delle decisioni ratificate della Parete (`.superpowers/sdd/task-1-decisioni-ratificate.md`). Quell'ondata ha chiuso **solo il buco che apriva lei** (`cassette`, `cassette_lavori`, `cassette_backfill_audit`) con `public.cassette_purge_lab` + deroga transaction-local nel trigger append-only + aggancio in `admin_delete_laboratorio` PRIMA di `DELETE FROM lavori` (migration `20260721090100`). Il fix generalizzato è stato **deliberatamente rimandato qui**, non «quando capita».
**Problema (preesistente e verificato, NON di quest'ondata):** `admin_delete_laboratorio` — unico percorso di hard-delete/erasure GDPR del repo — **non cancella 3 tabelle con `laboratorio_id`**: `fatture_outbox`, `fatture_sdi_eventi`, `credito_clienti_movimenti`. Restano orfane alla cancellazione del tenant (dato collegabile a persona non purgato; su `fatture_sdi_eventi` c'è anche la domanda retention-fiscale vs erasure art. 17).
**Fix di classe proposto (da ratificare con panel: architettura + tenant-lifecycle/sicurezza + **normativa**):**
1. funzione `<tabella>_purge_lab` generalizzata (o un registro delle tabelle da purgare) sullo stesso modello di `cassette_purge_lab`;
2. **asserzione da `information_schema` in CI** che enumera TUTTE le tabelle con colonna `laboratorio_id` e **fallisce** se una non è coperta da `admin_delete_laboratorio` (impedisce il ripetersi del difetto — è già successo con `lavori_partitario` droppata → funzione rotta in produzione, `20260702030000`);
3. copertura delle 3 tabelle orfane, con la decisione normativa esplicita su `fatture_sdi_eventi` (cancellare vs conservare per obbligo fiscale, ed eventuale anonimizzazione).
**Trappola nota:** `CREATE OR REPLACE FUNCTION admin_delete_laboratorio` azzera `proconfig` → ridichiarare sempre `SET search_path = public, pg_temp` in intestazione (hardening del 04/07 vive in un `ALTER FUNCTION` separato). Da aprire subito dopo la chiusura dell'ondata Parete.
...
### O3. Collaudo informale di Francesco (12/07/2026) — DEFERITO a fine sequenza
Francesco ha fatto un giro nella PWA post-Ondata 1 e ha visto diversi problemi. **Decisione esplicita (12/07, sera): la lista NON si raccoglie ora** — molti dei problemi visti oggi vivono su superfici v2.3 che le Ondate 2-4b riscrivono comunque (wizard, scheda lavoro, consegna), quindi si risolverebbero da soli prima di essere anche solo scritti. La raccolta/collaudo formale avviene al passo già previsto dalla sequenza (spec §12): dopo Ondata 4b, **collaudo Francesco → residui → audit multi-agente completo** → sotto-progetto 4. Nessuna azione richiesta prima di allora; non richiedere questa lista nelle sessioni intermedie.
...
## O6. Follow-up dal design/esecuzione Ondata 3a — Scheda lavoro v3 (13/07/2026, esecuzione SDD worktree `ondata-3a-scheda`)

Tutti non bloccanti per il merge di 3a. Tracciati dalla review finale whole-branch (Opus) e dai review per-task.

- **O6a — Nota del dentista (3b, richiede migration):** lo schema `lavori` NON ha un campo nota-scritta-dal-dentista (solo `note_interne` = nota privata del lab, e `richiedente_nome`/`richiedente_email` = contatti). Decisione Francesco (13/07): in 3a `note_interne` è mostrata come **nota onesta del laboratorio** (etichetta «Note (laboratorio)», niente attribuzione al dentista, tappabile per modifica). La **vera nota del dentista** = nuova colonna `note_dentista` (migration additiva) + compilazione dal **portale dentista** + display read-only nella scheda (attribuita al dentista) → **Ondata 3b**. Il componente DS `NotaDentista` è già pronto e intatto per questo scopo.
- **O6b — Estrarre loader condiviso `LavoroDettaglio`:** la query a 9 join + signing di `ddc.pdf_url`/`immagini` è ora **duplicata verbatim** in `src/app/(app)/lavori/[id]/page.tsx` e `src/app/(app)/lavori/[id]/modifica/page.tsx`. Estrarre `src/lib/lavori/get-dettaglio.ts` così che modifiche a schema/signing si applichino una volta sola (rischio drift silenzioso).
- **O6c — Cleanup file v2.3 morti:** dopo il montaggio della scheda-vista, `src/components/features/lavori/LavoroTimeline.tsx` e `TracciabilitaMaterialiBanner.tsx` non hanno più importer (quest'ultimo con logica reinlinata come `AvvisoTracciabilita`). NB: `LavoroFormClient.tsx` NON è morto — la route-ponte lo re-importa. Rimuovere i due file morti.
- **O6d — «Aggiungi prima nota» dalla scheda:** oggi la card «Note (laboratorio)» appare solo se `note_interne` è già presente; non c'è un affordance per aggiungere la prima nota dalla scheda (solo dal form ponte). Aggiungere un empty-state tappabile.
- **O6e — RifacimentoButton porta token DS-v2 in superficie v3:** riusato wholesale (`--elv/--prs/--t2/--primary`, `DM Sans` inline, `duration` dal modulo motion v2). Migrare a token/motion v3 quando si tocca il rifacimento.
- **O6f — A11y polish scheda v3:** `aria-label="Modifica scadenza"` sulla riga consegna (divergenza WCAG 2.5.3 label-in-name, oggi test-driven per non collidere con il bottone CONSEGNA — valutare «Modifica consegna» + query test più specifica); `:focus-visible` ring assente su `NotaLaboratorio` e sui bottoni inline-styled v3 (outline nativo presente) — gap sistemico condiviso con i componenti fratelli v3.
- **O6g — Date timeline non più mostrate nella scheda-vista:** `data_ingresso` e `data_consegna_effettiva` per-step (ex `LavoroTimeline`) non sono più visibili — sostituite dalla pill di stato. Scelta di design del mockup approvato, NON un requisito MDR (la compliance vive nel PDF DdC + segnale tracciabilità, entrambi preservati). Rivalutare se serve una vista storica/timeline in una fase successiva.
