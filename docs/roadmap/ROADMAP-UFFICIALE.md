...
**Ultimo aggiornamento:** 04 luglio 2026 — **B3 RISOLTO** (branch `worktree-b3-cicli-produzione`, 18 commit, non ancora mergiato su `main` — in attesa di conferma esplicita di Francesco): cicli di produzione ora generano automaticamente le fasi alla creazione lavoro, `TabProduzione` persiste realmente esito/non conformità (2 bug di regressione corretti). Review finale whole-branch "Ready to merge: Yes" dopo un fix critico (RPC atomica per il salvataggio batch fasi). Dettaglio: `memory/MEMORY.md` §0.
