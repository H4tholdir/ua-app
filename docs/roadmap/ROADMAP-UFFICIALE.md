...
**Ultimo aggiornamento:** 08 luglio 2026 — **REDESIGN DS v3 «Una cosa alla volta» IN CORSO** (pivot deciso da Francesco, B20-successivi in pausa). **Sotto-progetto 1 (fondamenta in codice) COMPLETATO** su worktree `.claude/worktrees/ds-v3-fondamenta`, branch `worktree-ds-v3-fondamenta` — **non ancora mergiato su `main`**. 10 task via `superpowers:subagent-driven-development`, tutti review-approvati: font Plus Jakarta Sans self-hosted, dizionario parole vietate + linter runtime, token v3 (con correzione WCAG ambra light → `#9A5C00`), CSS scoped `[data-ds="v3"]` (mai `:root`, coesistenza totale con v2.3), motion v3 (5 molle Apple + 8 coreografie), 5 suoni WAV provvisori + player Web Audio, haptic Android-only, estensione DS-compliance guard. Suite 707 passed | 4 skipped (era 670), `tsc`/`next build` puliti. Spec (legge di riferimento): `docs/superpowers/specs/2026-07-07-design-system-v3-una-cosa-alla-volta.md`. Dettaglio completo: `memory/MEMORY.md` §0. **Prossimo: sotto-progetto 2 (componenti core, spec §14.2)** — da avviare dopo merge del branch fondamenta o proseguendo nello stesso worktree.
