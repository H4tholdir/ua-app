...
**Ultimo aggiornamento:** 12 luglio 2026 (notte) — **DS v3 «Il cuore» — ONDATA 2 (WIZARD NUOVO LAVORO): DESIGN COMPLETATO E APPROVATO** (spec `docs/superpowers/specs/2026-07-12-ds-v3-il-cuore-ondata-2-wizard-design.md`, gate chiuso «va tutto bene»). Decisioni chiave (ricerca 3 advisor: odontotecnico 2026 + UX quick-entry + explorer interno, convergenti): **tassonomia granulare 38 tipi in 9 famiglie** (costante `tipi-lavoro.ts` — macro-slug in `tipo_dispositivo`, label granulare in `descrizione` che precompila la DdC, classe di rischio default per tipo, alias di gergo in ricerca, tabella ratificata così com'è) · **nuovo macro `bite_splint`** (unica migration, additiva al CHECK) · **fotocamera in-wizard** · architettura A (route + passi client + localStorage 24h) · sheet «Nuovo dentista» a 3 campi (solo nome+cognome obbligatori — la DdC All. XIII chiede solo il nome del prescrittore) · alias paziente opzionale · fail-soft accessori · zero API nuove. In-ondata anche: validazione enum 422 su POST/PATCH lavori (B2) e consolidamento mappe label (B4). **FEATURE RATIFICATA per sotto-progetto 4 (B6): tassonomia come motore MDR** — scheda di fabbricazione precompilata coi materiali attesi per tipo, suggerimento automatico del ciclo di produzione, pipeline di promozione dei testi liberi ricorrenti a tipi ufficiali. Segnalazioni triaged in BACKLOG-TECNICO §O4 (ClienteComboBox, listino.categoria, «completa dati fiscali alla prima fattura»). **PROSSIMO: piano Ondata 2 via `superpowers:writing-plans` → esecuzione SDD in worktree**, poi Ondata 3 Scheda e 4b UI Consegna.
