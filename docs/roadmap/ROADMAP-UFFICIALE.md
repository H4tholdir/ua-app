...
**Ultimo aggiornamento:** 08 luglio 2026 — **REDESIGN DS v3 «Una cosa alla volta» IN CORSO — Sotto-progetto 2 (componenti core) COMPLETATO su branch, in attesa di approvazione visiva del catalogo** (pivot deciso da Francesco, B20-successivi in pausa). **Sotto-progetto 2** su worktree `.claude/worktrees/ds-v3-componenti`, branch `worktree-ds-v3-componenti` — **NON ancora mergiato su `main`**: il gate ufficiale (spec §14.2) è l'approvazione visiva del catalogo pubblico da parte di Francesco, poi la review finale whole-branch, poi il merge previa conferma esplicita. 16 task via `superpowers:subagent-driven-development`, tutti review-approvati (diversi dopo un round di fix): tutti e 27 i componenti §5 completi in `src/components/ds/` (TastoPrimario/Secondario/Tondo, LinkQuieto, PillTempo/PillStato/PillFase, TastoPiu, Avatar, TileScelta/Nuovo, RigaCerca, Pila, StrisciaStato, CardLavoro, CardInfo/RigaDato, RigaFase/CheckTondo, Sheet, DialogConferma, Avviso, Skeleton, Vuoto, Campi testo/numero/data, BarraMateriale, EroeTuttoAPosto, CardUAHaFatto, NotaDentista, GiornoAgenda/RigaAgenda, PillVoce); catalogo pubblico `/ds-v3-catalogo` (13 sezioni, toggle tema, in `PUBLIC_ROUTES`) con 6 screenshot QA in `docs/design/catalogo-v3/` (390/768/1280 × light/dark); il QA visivo (Task 15) ha trovato e fissato 3 bug reali invisibili a jsdom (CardLavoro overflow 390px, Avviso hydration mismatch, RigaAgenda overlap 390px). Suite **1058 passed | 4 skipped** (era 707 alla baseline `main`, +351), `tsc`/`next build` puliti (`/ds-v3-catalogo` statico). Spec (legge di riferimento): `docs/superpowers/specs/2026-07-07-design-system-v3-una-cosa-alla-volta.md`. Dettaglio completo: `memory/MEMORY.md` §0. **Prossimo:** review finale whole-branch → approvazione catalogo Francesco → merge su `main` → **sotto-progetto 3 «Il cuore»** (Home, pile, wizard, scheda lavoro, consegna — spec §14.3).
