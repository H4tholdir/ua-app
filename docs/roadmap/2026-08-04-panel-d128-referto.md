...
🔄 **Correzione a questo stesso referto, fatta il 04/08 poche ore dopo averlo scritto — l'errore era mio e va detto perché manda il lavoro nel posto sbagliato.** Sopra avevo scritto, seguendo un advisor, che la sede del contratto laboratorio↔UÀ fosse la tabella `sub_processors`, «disegnata apposta e con zero lettori e scrittori». **Sono DUE tabelle diverse e nessuna delle due frasi era esatta:**
- **La sede giusta è `data_processing_agreements`**, che la controparte giusta **ce l'ha già prevista e non l'ha mai vista**: `provato:` il vincolo ammette esattamente due valori — `CHECK (tipo_controparte = ANY (ARRAY['dentista','sub_responsabile']))` — e le righe `'sub_responsabile'` sono **zero**.
- **`sub_processors` è un'altra cosa: è l'elenco dei fornitori di UÀ**, e **non è vuota**: `provato:` **6 righe** già dentro (Supabase, Vercel, Resend, Stripe, AdE/Sogei, Ministero Salute), con `dpa_firmato_at` **NULL su tutte e sei**. È **popolata e mai LETTA dall'applicazione** — e porta un difetto suo: l'elenco dei tre fornitori che il contratto mostra al dentista è **scritto a mano dentro il modello PDF** (`DpaTemplate.tsx:207-209`) invece di venire da quella tabella, che è poi la ragione per cui aggiungere un fornitore obbliga oggi a riemettere tutto (v. §3 e la condizione **C8** del referto sui sub-responsabili).
