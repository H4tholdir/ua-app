-- supabase/migrations/20260704150000_hotfix_salva_fasi_ciclo_atomico_kept_ids.sql
-- UÀ Hotfix CRITICO — salva_fasi_ciclo_atomico() soft-deletava ogni fase
-- appena inserita, nella stessa chiamata che la creava.
--
-- Causa: il ramo INSERT (fase nuova, senza id) non catturava l'id generato
-- né lo aggiungeva a v_kept_ids — a differenza del ramo UPDATE (fase
-- esistente), che lo fa correttamente. Lo step finale di pulizia soft-delete
-- tutto ciò che non è in v_kept_ids: per un batch con fasi nuove, quell'array
-- restava vuoto (o privo degli id appena creati), quindi la pulizia
-- cancellava anche le righe appena inserite nella stessa transazione.
--
-- Scoperto il 04/07/2026 preparando la copertura di test comportamentale
-- reale per il finding B18.6 (la funzione era testata solo con mock della
-- RPC, mai col suo comportamento SQL reale). Verificato in transazioni
-- BEGIN/ROLLBACK sul DB live (mai commit) — nessun dato reale perso: i
-- cicli reali del lab Filippo non erano mai stati toccati da questa RPC
-- dal deploy di B3 (introdotta tardi, durante il fix della review finale,
-- in sostituzione della logica manuale insert/update/delete).
--
-- Fix: RETURNING id INTO v_fase_id sull'INSERT, poi append a v_kept_ids —
-- stesso trattamento già riservato al ramo UPDATE.
