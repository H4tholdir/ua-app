-- Fix: admin_delete_laboratorio() — risolve FK constraint per utenti admin_sistema
-- Prima: la funzione saltava la delete degli admin_sistema ma il loro laboratorio_id
--        puntava ancora al lab → FK utenti_laboratorio_id_fkey crash
-- Fix:   UPDATE utenti SET laboratorio_id = NULL WHERE ruolo = 'admin_sistema'
--        prima di DELETE laboratori, così il FK è soddisfatto
...
BEGIN
  ...
  DELETE FROM lavori_partitario       WHERE laboratorio_id = p_lab_id; GET DIAGNOSTICS v_n = ROW_COUNT; v_counts := v_counts || jsonb_build_object('lavori_partitario', v_n);
