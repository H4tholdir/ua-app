-- supabase/migrations/20260808112700_atto_unico_coppia_indivisibile.sql
-- Ondata «correggi e rifai la dichiarazione», Task C-ter — LA COPPIA È INDIVISIBILE.
-- Piano: docs/superpowers/plans/2026-08-08-correzione-e-riemissione-atto-unico.md, «Task C» (C-ter).
--
-- 🛑 MIGRATION NUOVA, e non una modifica a `20260808103515_atto_unico_p_nuova_irrigidita.sql`
--    né a `20260808093513_correggi_e_riemetti_atomica.sql`: sono già applicate e
--    registrate nel ledger, toccarle lo disallineerebbe dal catalogo.
--
-- 📌 IDIOMA: `DROP` → `CREATE` → `REVOKE` → `GRANT` → `COMMENT`, lo stesso della
--    migration precedente, e per due ragioni. (1) `pg_get_functiondef` non
--    conosce le modifiche parziali: una funzione si sostituisce tutta o niente,
--    quindi il file porta il corpo INTERO ed è leggibile come la verità di
--    quella funzione. (2) I due file consecutivi si leggono allo stesso modo.
--    ⚠️ Il brief chiedeva una `CREATE OR REPLACE`; con la stessa firma le due
--    forme arrivano allo stesso posto, ma NON sono equivalenti sull'ACL — dopo
--    un `DROP`+`CREATE` Postgres concede `EXECUTE` a `PUBLIC` per impostazione
--    predefinita, quindi qui sotto il `REVOKE` è PORTANTE, non cerimoniale.
--    Con `CREATE OR REPLACE` l'ACL esistente sopravvive e il `REVOKE` sarebbe
--    una seconda rete. Si è scelta la forma in cui la rete è quella dichiarata.
--
-- ═══ LA PORTA CHE QUESTA MIGRATION CHIUDE ════════════════════════════════════
--
-- Il Task C-bis ha tolto `numero_ddc` dalle mani del chiamante e lo ha reso
-- DERIVATO da `anno_ddc` + `progressivo_ddc`. Ma la coppia da cui lo deriva NON
-- era protetta: se ne arriva UNA SOLA, l'altra se la prende `jsonb_populate_record`
-- dalla dichiarazione VECCHIA, in silenzio — e il numero che ne esce è
-- **plausibile e sbagliato**. È la seconda metà di C1, non un difetto nuovo.
--
-- 🔴 Il difetto ha DUE versi, ed è per questo che la regola è `XOR` e non
--    l'implicazione «pretendi l'anno quando c'è il progressivo»: quella ne
--    chiuderebbe uno solo. provato: sull'oggetto VIVO, in transazione annullata,
--    con la dichiarazione vecchia a `2098 / 998001`:
--
--    ① p_nuova = {'progressivo_ddc': 999005}   ← senza anno
...
--
--    ② p_nuova = {'anno_ddc': 2099}            ← senza progressivo
...
--                                                        e per il 2099 nessuno l'ha mai prenotato
--
--    🔑 Il ② non collide con niente: la coppia `(lab, 2099, 998001)` è libera.
--    Un documento a valore legale con un numero che nessun contatore ha emesso.
--
-- ⚠️ PERCHÉ NESSUNA SONDA L'AVEVA PRESO, e vale più della correzione: la fixture
--    del Task B ha la dichiarazione vecchia nel **2099**, cioè nello stesso anno
--    che le sonde passavano. L'anno ereditato **coincideva per combinazione** e
--    l'asserzione sul numero restava verde senza aver mai guardato l'anno.
--    Sedici sonde fra Task B e C-bis, tutte cieche sulla stessa metà.
--    ➡️ Le sonde di questo compito girano su una fixture con la vecchia nel
--    **2098** — e il 2098 sta DENTRO l'INSERT, non in un `UPDATE` aggiunto dopo.
--    *Una fixture che sceglie i propri valori può nascondere il difetto che
--    dovrebbe mostrare.*
--
-- ═══ PERCHÉ È SICURO IMPORLA, E PERCHÉ ORA ═══════════════════════════════════
--
-- · Mandarne ZERO già fallisce rumorosamente: la nuova eredita entrambe dalla
--   vecchia e l'INSERT sbatte su
--   `dichiarazioni_conformita_laboratorio_id_anno_ddc_progressiv_key` (23505).
--   Quel caso resta com'è: non è questa guardia a doverlo prendere, ed è la
--   ragione per cui la regola è «o tutte e due, o nessuna» e non «tutte e due
--   sempre».
-- · L'unico chiamante legittimo le manda SEMPRE tutte e due:
--   `costruisciDichiarazione` costruisce `ddc` con `numero_ddc`, `anno_ddc` e
--   `progressivo_ddc` insieme (`src/lib/pdf/generate-ddc.ts:234-236`).
-- · E la funzione NON HA ANCORA CHIAMANTI (provato: `grep -rn
--   correggi_e_riemetti_atomica src/` → solo `src/types/database.types.ts`).
--   Irrigidire un contratto senza consumatori costa una `RAISE`; irrigidirlo
--   dopo il Task C costerebbe il contratto PIÙ il suo chiamante.
--
-- ═══ CIÒ CHE QUESTA MIGRATION NON TOCCA (R-E2) ═══════════════════════════════
--
-- Il resto del corpo è ribattuto IDENTICO al catalogo vivo: unico blocco nuovo
-- è la guardia della coppia, e il `COMMENT` che la dichiara. Restano aperti e
-- RIFERITI, non corretti di nascosto:
--   · `riemetti_ddc_atomica` porta ENTRAMBI i buchi del C-bis PIÙ questo, ed è
--     quella con un chiamante vivo (`riemettiDdC`, `generate-ddc.ts:463`). Da
--     qui in poi le due funzioni SEMBRANO gemelle e non lo sono: roadmap, la
--     riga 26 della coda.
--   · `p_correzioni` non rifiuta mai il VUOTO (C2, del Task C).
--   · `{"anno_ddc": null, "progressivo_ddc": N}` supera questa guardia — le due
--     chiavi CI SONO — e muore più sotto su `23502 … column "numero_ddc"`:
--     fail-closed e rumoroso, ma con un messaggio che nomina una colonna che il
--     chiamante non ha toccato. È il rilievo M2 della revisione del C-bis, e
--     resta FUORI da questo mandato: la regola qui è sulla PRESENZA delle
--     chiavi, non sui loro valori.
