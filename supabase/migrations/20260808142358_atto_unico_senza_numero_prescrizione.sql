...
-- Ondata «correggi e rifai la dichiarazione», Task C-quinquies — D319:
-- IL NUMERO DI PRESCRIZIONE ESCE DALLE VOCI CORREGGIBILI.
-- Decisione: docs/design/decisions/2026-07-28-wizard-ondata-b-decisioni.md,
--            centotrentottesima tornata (D319).
--
-- 🛑 MIGRATION NUOVA, e non una modifica alle TRE gia' applicate e registrate nel
--    ledger (20260808093513, 20260808103515, 20260808112700): toccarle
--    disallineerebbe i file dal catalogo vivo.
--
-- 📌 IDIOMA: `DROP` -> `CREATE` -> `REVOKE` -> `GRANT` -> `COMMENT`, lo stesso
--    delle tre precedenti. (1) `pg_get_functiondef` non conosce le modifiche
...
--    Postgres concede `EXECUTE` a `PUBLIC` per impostazione predefinita —
--    misurato dalla revisione del C-ter.
--
-- ═══ CHE COSA CAMBIA, E PERCHE' ══════════════════════════════════════════════
--
-- `p_correzioni` passa da OTTO chiavi a SETTE: esce `numero_prescrizione`.
-- La ragione e' NORMATIVA, non di prodotto. L'Allegato XIII punto 1, sulla
-- prescrizione, chiede DUE cose — «il nome della persona che ha prescritto il
-- dispositivo… e, se del caso, il nome dell'istituzione sanitaria» e «le
-- caratteristiche specifiche del prodotto indicate nella prescrizione». Un
-- NUMERO, codice o identificativo della prescrizione non compare fra gli otto
-- trattini: non e' un contenuto dovuto, quindi non sta sul documento e non c'e'
-- niente da correggere. Le SEI voci a schermo rimaste sono tutte dovute.
--
-- 🔑 E c'e' gia' il campo giusto per lo scopo che quel numero sembrava servire:
--    ritrovare la prescrizione di carta e' il mestiere di
--    `lavori_prescrizioni.fonte_tipo`/`fonte_riferimento` (ondata B).
--
-- ═══ R-P6 · IL NOME ESCE DA UN'ALLOWLIST, QUINDI PORTA LA SUA DESTINAZIONE ═══
--
-- La destinazione di `numero_prescrizione` e' **NESSUNA, e per scelta**: da D319
-- non alimenta piu' la dichiarazione. Le colonne NON si cancellano — non si
-- perde niente — ma nessuna di loro alimenta piu' un documento:
--
--   · `lavori.numero_prescrizione` — dopo questo compito **non ha piu' ne'
--     lettori ne' scrittori**. L'ultimo lettore era `generate-ddc.ts:257`
--     (`prescrizione_id`), tolto nello stesso compito; l'ultimo scrittore era
--     l'`UPDATE` qui sotto. ⚠️ Resta SELEZIONATA da due rotte di fatturazione
--     (`api/fatture/batch/route.ts:113`, `api/fatture/[id]/xml/route.ts:97`) che
--     caricano la riga intera del lavoro e non la usano: e' cio' che la fara'
--     sembrare viva al prossimo che legge.
--   · `lavori_prescrizioni.numero_prescrizione` — 🛑 **NON e' orfana, e il brief
--     di questo compito diceva il contrario**: `POST /api/lavori` la accetta e la
--     valida (`api/lavori/route.ts:234-240` -> `componiSnapshot` ->
--     `lavoro_crea_atomico`), il clone del rifacimento la propaga
--     (`20260807185858:177-180`) e `prescrizione-mapper.ts:291` la legge nel tipo
--     di dominio. Cio' che e' vero e' piu' stretto: **da D319 non alimenta piu'
--     la dichiarazione**, e il wizard non ha la casella per scriverla.
--     ➡️ Chiudere quella porta d'ingresso e' FUORI da questo mandato: RIFERITO
--     (R-E2), non corretto di nascosto.
--   · `dichiarazioni_conformita.prescrizione_id` — perde il suo UNICO scrittore
--     (`generate-ddc.ts`) e non aveva altri lettori: colonna orfana da oggi.
--     `provato:` 0 righe valorizzate su 6.
--
-- ⚠️ Il progetto ha gia' pagato questo esatto errore (R34: una colonna rimasta
--    senza scrittori e otto lettori che «sorvegliano uno stato che niente
--    produce»). Le righe qui sopra esistono per questo.
--
...
DECLARE
  ...
  -- ⚖️ ERANO OTTO FINO A D319 (08/08/2026): `numero_prescrizione` e' uscito da
  --    `c_su_lavori`, e con lui la sua riga dall'`UPDATE` piu' sotto. I due posti
  --    si tolgono INSIEME: togliere il nome e lasciare la riga del `SET`
...
BEGIN
  ...
  -- ⚖️ Da D319 è QUESTA riga a rifiutare `numero_prescrizione`, e il messaggio
  --    porta con sé l'elenco delle sette ammesse: chi lo manda legge subito che
  --    non è più una voce del documento, invece di vederlo sparire.
  ...
  -- ─── ④a le CINQUE voci che vivono su `lavori` ──────────────────────────────
  -- ⚖️ Erano SEI fino a D319: `numero_prescrizione` è uscito dall'allowlist e da
  --    questo `SET` nello stesso gesto.
...
COMMENT ON FUNCTION public.correggi_e_riemetti_atomica(uuid,uuid,uuid,jsonb,jsonb,timestamptz) IS
  ...
  'p_correzioni ammette SETTE chiavi, e ognuna ha una destinazione: '
  'richiedente_nome · paziente_id · paziente_nome_snapshot · '
