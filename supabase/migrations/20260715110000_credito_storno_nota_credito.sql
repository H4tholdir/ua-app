-- Task 4 (Nota di Credito TD04): tipo movimento credito 'storno' + Blocco 3
-- della RPC emetti_nota_credito_atomica (credito cliente se la fattura
-- originale era stata incassata).
-- Spec: docs/superpowers/specs/2026-07-14-nota-credito-td04-design.md
-- Additiva rispetto a 20260715100000_emetti_nota_credito_rpc.sql (Task 3 già
-- committato): CREATE OR REPLACE con il corpo completo aggiornato.
--
-- NOTA sulla fonte dell'importo incassato (verificata sul DB reale):
-- fatture.importo_pagato ESISTE (migration 20260702185348 §4, NOT NULL,
-- derivata via trigger ricalcola_pagamento_fattura da pagamenti attivi +
-- applicazioni di credito). Il TODO di Task 3 suggeriva fatture_pagamenti:
-- quella tabella è lo scadenzario legacy MAI scritto dal flusso B2 (nessun
-- INSERT in src/) — sommarla darebbe sempre 0. Qui si ricalcola inline la
-- STESSA formula del trigger (pagamenti attivi + applicazioni), così il
-- risultato non dipende dalla freschezza della colonna derivata.

-- ------------------------------------------------------------
-- 1. CHECK tipo: accetta 'storno'
-- ------------------------------------------------------------
...
ALTER TABLE public.credito_clienti_movimenti
  ...
  CHECK (tipo IN ('eccedenza','applicazione','rimborso','storno'));

-- ------------------------------------------------------------
-- 2. CHECK shape per-tipo: ramo dedicato per 'storno'
--    (nessun pagamento sorgente; fattura_id = originale stornata, per audit;
--     mai legato a un lavoro)
-- ------------------------------------------------------------
ALTER TABLE public.credito_clienti_movimenti
...
ALTER TABLE public.credito_clienti_movimenti
  ADD CONSTRAINT credito_clienti_movimenti_check CHECK (
    ...
    (tipo = 'storno'       AND pagamento_id IS NULL AND fattura_id IS NOT NULL AND lavoro_id IS NULL)
  );

-- ------------------------------------------------------------
-- 3. registrato_da nullable: NULL = movimento generato dal sistema
--    (la RPC non ha un utente chiamante — è service_role only)
-- ------------------------------------------------------------
ALTER TABLE public.credito_clienti_movimenti
  ALTER COLUMN registrato_da DROP NOT NULL;
COMMENT ON COLUMN public.credito_clienti_movimenti.registrato_da IS
  'Utente che ha registrato il movimento. NULL = generato dal sistema (es. storno da RPC emetti_nota_credito_atomica).';

-- ------------------------------------------------------------
-- 4. RPC: corpo completo con Blocco 3 riempito
-- ------------------------------------------------------------
