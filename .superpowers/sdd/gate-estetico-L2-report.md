...
| 5 | Colore, contrasto, tema | ✅ / ⚠️ | ✅ / ⚠️ | ✅ / ⚠️ | testo: tutto AA nei due temi (§3). ⚠️ dark = **R5**, il modellato dei gancetti si appiattisce |
...
|---|---|---|---|---|---|
| **R1** | Passo fra le file del muro: 5× è un residuo, la sua giustificazione nel CSS non è più valida (tile ora fisso a 138px). Proposta: 4× da `@container 660px` in su → −250px su 24 cassette a 1280. ⚠️ **Il compromesso da nominare a Francesco:** la stanza parete della home ha un container troppo stretto perché la soglia scatti, quindi resterebbe a 5× — a **parità di larghezza di finestra (768)** si avrebbero **due ritmi diversi**, 62px su `/cassette` e 86px nella home. Oggi i valori sono tre ma nessuno se li vede accanto. Può cambiare la sua risposta. **In ogni caso il commento `ds-v3.css:1058-1067` va corretto: descrive una geometria superata.** | 2, 10 | `src/app/ds-v3.css:1058-1077`, `:1277` | **P1 — decisione di Francesco** | §4.1 + backlog ondata |
...
| **R5** | Gancetti in dark: il **modellato interno** del gradiente crolla. Non è un problema di stacco sul fondo (nessun gancetto si legge come silhouette: in light lo stop chiaro sta a 1.04:1 sul muro) — il gancetto prende forma dal proprio chiaroscuro, e lo scarto `hi`↔`lo` passa da **3.21:1 in light** a **1.68:1 in dark**. Sopra la maglia scura il pezzo si appiattisce in una macchia. Il dark *è* stato considerato (l'override `--gan-metal-*` esiste), ma è stato portato troppo giù: il gesto «appeso al muro» — l'idea su cui poggia tutto il redesign — in dark non si legge. Prova a occhio: `390-{light,dark}-cima-muro-gancetti.png`, gli stessi tre ganci nei due temi. | 5 | `ds-v3.css:488-492` | P2 | backlog ondata |
