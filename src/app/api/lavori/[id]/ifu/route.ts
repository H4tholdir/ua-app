...
import { generateIFU } from '@/lib/pdf/generate-ifu'
...
export async function GET(
  ...
  return withServerTiming(async (t) => {
    ...
    // → 401 Unauthorized (getLabContext fail-closed, vedi lab-context.ts).
    ...
    try {
      const buffer = await generateIFU(lavoro_id, context.laboratorioId)
      return new NextResponse(new Uint8Array(buffer), {
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="IFU_${lavoro.numero_lavoro}.pdf"`,
        },
      })
    } catch (e: unknown) {
    ...
    //    la sua ragione è scritta nel suo file ed è stata verificata.
      console.error('[ifu] generazione fallita:', e)
      return NextResponse.json(
        { error: 'Non è stato possibile generare il documento (istruzioni per l’uso). Riprova fra poco.' },
        { status: 500 },
      )
    }
  })
