...
// definizione già chiusa, sempre `fatto`.
 ...
 * `--faint`, maiuscolo) · righe: `CheckTondo` Ø 30 sempre `fatto` + `nome`
 * 16.5/700 `--ink` + `sub` 14/500 `--muted` opzionale.
 */
export function CardUAHaFatto(props: { voci: Array<{ nome: string; sub?: string }> }) {
  const { voci } = props
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spazio.sm }}>
        {voci.map((voce, indice) => (
          <div key={indice} style={{ display: 'flex', alignItems: 'center', gap: spazio.m }}>
            <CheckTondo fatto diametro={DIAMETRO_CHECK} />
